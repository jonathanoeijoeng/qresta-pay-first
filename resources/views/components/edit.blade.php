...
@php
// Logika untuk menentukan tag: jika ada route maka <a>, jika tidak maka <button>
        $tag = $route ? 'a' : 'button';

        // Default attributes untuk masing-masing tag
        $extraAttributes = $route
        ? ['href' => $route]
        : ['type' => 'button'];
        @endphp

        <{{ $tag }} {{ $attributes->merge(array_merge($extraAttributes, [
            'class' => 'w-6 transform inline-block group focus:outline-none'
            ])) }}>
            <svg class="text-yellow-500 cursor-pointer fill-current group-hover:scale-125 transition duration-200"
