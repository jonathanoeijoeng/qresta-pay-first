    ...
    public function mount()
    {
        ...
        if (session('active_order_id')) {
            ...
            if ($sessionTableId !== session('customer_table_id')) {
                ...
                session()->forget(['active_order_id', 'merging_order_id']);
            }
        }
    ...
    public function render()
    {
        ...
        $categories = Category::query()
            ...
            ->whereHas('menus', function ($q) use ($branchId, $searchTerm) {
                $q->where('menus.name', 'ilike', $searchTerm)->whereHas('branches', function ($bq) use ($branchId) {
                    $bq->where('branch_id', $branchId)->where('branch_menu.is_available', true);
                });
            })
            // 2. FILTER MENU: Ambil detail menu yang sesuai (untuk ditampilkan di dalam kategori)
            ->with([
                'menus' => function ($query) use ($branchId, $searchTerm) {
                    $query
                        ->select('menus.*', 'branch_menu.price as branch_price', 'branch_menu.is_available as branch_available')
...
<div>
                ...
                <div
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex flex-col">
                            <span
                                class="text-[10px] text-brand-600 font-black uppercase tracking-widest leading-none">Mode
                                Tambah Pesanan</span>
                        ...
                        <div
                            ...
                            <div class="flex-1 p-3 flex flex-col justify-between">
                                <div>
                                    ...
                                    <p class="text-[10px] text-zinc-500 line-clamp-2 mt-0.5">{{ $menu->description }}
                                    </p>
                                </div>
                                <div class="flex justify-between items-center">
                                    ...
                                    <button wire:click="openNoteModal({{ $menu->id }})" wire:loading.attr="disabled"
                                        {{-- Saat
    ...
    @endif
    @if ($showNoteModal)
        <div class="fixed inset-0 bg-black/50 z-[10000] flex items-center sm:items-center justify-center p-2 sm:p-4">
            <div
                class="bg-white dark:bg-zinc-300 w-full max-w-md rounded-3xl sm:rounded-3xl p-6 
